  ...
  return {
    ...
    validateUsernameAndPassword : function() {
      ...
      if(errorText) {
        ...
        kony.application.showLoadingScreen("sknBlockUI","Loading",
    ...
    doLogin : function(){
      var authClient = null;
      var providerName = "ShoppingIdentity";
      var self = this;
      var username = "suresh.jallipalli@hcl.com";
      var password = "voltMX@1234";
      try {
        var sdkClient = new kony.sdk.getCurrentInstance();
        authClient = sdkClient.getIdentityService(providerName);
      } catch (exception) {
        alert("There seems to be some error while Logging in. Please try again." );
      }
      authClient.login({"userid": username,
                        "password": password},
                       function(response) {
        ...
        alert("There seems to be some error while Logging in. Please try again.");
        kony.print("There seems to be some error while Logging in. Please try again."+ error);

        kony.application.dismissLoadingScreen();
      });
    },
    getShoppingLists : function(){
      var serviceName = "ShoppingLists";
      var sdkClient = new kony.sdk.getCurrentInstance();
      var integrationObj = sdkClient.getIntegrationService(serviceName);
      var operationName = "getAllLists";
      var data= {};
      var headers= {};
      integrationObj.invokeOperation(operationName, headers, data, this.shoppingListSuccess, this.shoppingListFailure);
    },
    shoppingListSuccess : function (res){
      var StoreUtil = require('com/shopping/Login/StoreUtil');
      kony.print("Data Returned::"+ res);
      res.groups = [];
      res.listItems = [];
      StoreUtil.storeAllLists(res.lists);
      StoreUtil.storeAllGroups(res.groups);
      StoreUtil.storeAllListItems(res.listItems);
      //alert(StoreUtil.getShoppingListData());
      var nav = new kony.mvc.Navigation("frmShoppingList");
      nav.navigate();
    },
    shoppingListFailure:function (err){
      kony.print("Error Getting Lists ::"+ err);
      alert("There seems to be some error while Logging in. Please try again.");
      kony.application.dismissLoadingScreen();
    }
  };
