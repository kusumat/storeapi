define(function() {
  ...
  return {
    constructor: function(baseConfig, layoutConfig, pspConfig) {
      ...
      this.view.lblHint.text = "* This will allow multiple documents (limit 10) to be uploaded (max "+bytesToSize(gblFileMaxUploadSize, true)+" total) and attached to the hearing request.";
    ...
    setComponentData: function() {
      ...
      this.view.lblAgrreateFileSize.text = "Total file size: " + bytesToSize(TotalFileSize, true);
      this.view.flxMainContainer.flxScrollGroupDocuments.sgmDocumentsUploaded.setData(data);         
    },
  ...
  getDocumentsListFailure:function(error){
      ...
      if(indexOfFileType !== -1) {
        ...
        reader.addEventListener("load", function() {
          if(reader.result !== null && reader.result !== undefined) {
            ...
            self.setFileName(fileName, readResult, fileSize);
          }
        });
      }
      else {
    ...
    setFileName:function(fileName, fileContent, fileSize) {
      if (hearingRequest.uploadDocuments.length === 10) {
        ...
        if (TotalFileSize + fileSize <= gblFileMaxUploadSize)
        {
          ...
          var row = {"lblTextFileName": fileName,
                     ...
                     "lblTextFileSize": bytesToSize(fileSize, false), 
                     "btnMgIconDel": {onClick: this.removeSegmentDocument.bind(this, "index")}, 
                     "flxBtnDel": {onClick: this.removeSegmentDocument.bind(this, "index")}
                    };
          ...
          currentFrm.view.puInformationDialog.lblDescText = "Adding this document: "+'"'+fileName+ '"'+", with a file size of " + bytesToSize(fileSize, true) + ", would exceed the total allowable file size of "+bytesToSize(gblFileMaxUploadSize, true)+".\n\nPlease remove a file uploaded or reduce the file size to be lesser than "+ bytesToSize(gblFileMaxUploadSize, true) + ".";
