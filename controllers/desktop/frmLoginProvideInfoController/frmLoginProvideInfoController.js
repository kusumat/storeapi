define({ 
  ...
  onNavigate:function() {
    gblSettings(this);
    this.view.snLoginAditionalInfo.additionInfoBackButtonOnClick = this.goBack;
    this.view.snLoginAditionalInfo.additionalInfoContinueButtonOnClick = this.ContinueButtonOnClick;
    this.refreshAppellantVsARComponents();
    this.view.onBreakpointChange = this.onBreakpointChange;
    this.view.preShow = this.preShow;
    addToFlow(this.view.id, this.view);
    if(gblRepresentingOption == "APP") {
      this.view.flxBoxLoginContainer.height = 800;
    }
    else
    {
      this.view.flxBoxLoginContainer.height = 1075;
    }
  ...
  goBack:function() {
    kony.print("inside goBack");
    var ntf = new kony.mvc.Navigation("frmLoginBeforeStart");
    ntf.navigate();
  },
  ContinueButtonOnClick:function() {
    kony.print("inside ContinueButtonOnClick");
	if(gblUserInfoValidated) {
